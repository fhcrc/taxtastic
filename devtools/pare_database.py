...
def main():
    ...
    parser.add_argument('-o', '--output-file', default=sys.stdout, type=argparse.FileType('w'))

    a = parser.parse_args()
    with a.keep_taxids as fp:
        lines = (i for i in fp if not i.startswith('#'))
        keep_ids = frozenset(i for i in re.split(r'[\s\n]+', ''.join(lines), flags=re.MULTILINE) if i)
    ...
    with tempfile.NamedTemporaryFile() as tf:
        ...
        cur.execute("DELETE FROM nodes WHERE tax_id NOT IN {0}".format(ic), list(keep_taxa))
        print >> sys.stderr, "pruning names"
        cur.execute("DELETE FROM names WHERE tax_id NOT IN {0}".format(ic), list(keep_taxa))
