...
\begin{defn}
  ...
  Define $\cut(T)$ to be the colors of $\col$ cut by the root edge of $T$ as it sits inside $S$.
\end{defn}

Let $\subcol$ be a solution of Problem~\ref{prob:subcolor} for $(S, \col)$ and $T$ a rooted subtree of $S$.
Now, the image $C = \subcol(L(T))$ is a subset of $\cut(T)$.
The subcoloring restricted to $L(T)$ must be optimal for the colored rooted tree $(T, \col \|_{L(T)})$.
BUT-- we also have to make it clear that the root must be a certain color as well, no?

\begin{defn}
  Given a rooted subtree $T$ of a rooted colored tree $(S, \col)$, define a subsolution for a rooted subtree $T$ and $Y \subset \cut(T)$ to be a maximal element $\phi(Y)$ of $\subcolS_{L(T)}(\col)$ which includes at least one taxon with each color in $Y$.
\end{defn}
...
% This corresponds with the original definition of $Y$:
% \begin{lemma}
% \[
%   (Y_1 \cup Y_2) \cap K = (C_1 \cup C_2) \cap K
% \]
% \end{lemma}
% \begin{proof}
%   $\subset$ is clear.
...
\begin{defn}
  Let the coloring set $\colorS(T, C)$ where $T$ is a phylogenetic tree and $C$ is a set of colors to be the set of maps $\chi: Y \rightarrow C$ where $Y \subset L(T)$.
\end{defn}


Kicked out:
\begin{enumerate}
  \item $r_1 \in Y_1 \cap Y$ and $r_2 \in Y_2 \cap Y$
(this is too strong: we are now not worrying about the root ``state'' unless it matters)
\end{enumerate}

Don't forget!
$Y_1$ and $Y_2$ need not sit in $Y$.




Let $Y^c$ be $Y$'s complement in $K$.
...
Explicit descripton:
We start with our $Y$.
We take every $O_1$ and $O_2$ subsets of $O$ such that $|O_1 \cap O_2| \leq 1$ and $(O_1 \cup O_2) \cap Y^c = \emptyset$.
We also need to make sure that we don't have $O_1$ and $Y$ intersecting with a different color when $O_1$ and $O_2$ do.
Then $Y_i = J_i \cup O_i$.

The solution for $Y$ is then the union of the best for each $Y_i$.
...
- $R = K_1 \symmdiff K_2$, in which case $Y_1$ and $Y_2$ are disjoint, which gives an $|R| 2^{|R|}$.
- $U = Y \cap K_1 \cap K_2$, in which case we have $|U| 2^{|U|}$.

So that could be a $2^{|K_1 \cap K_2|}$ operation.
We have to do this for every $(\rho, Y)$, meaning that the whole machine will have complexity $K \times 2^{|K| + |K_1 \cap K_2|}$.
...
Say we would like to find a solution for $(\rho, Y) \in \opt{K} \times 2^K$.
If a color is in
Let $R = Y \cap (K_1 \symmdiff K_2)$; for these colors we are Really sure where they should come from.
We take every $Y_1,Y_2$ pair such that
- $Y_1 \subset K_1$
- $Y_2 \subset K_2$
-
why would we exclude something that is not cut?

Let $U = Y \cap K_1 \cap K_2$; these are the colors which we don't know which tree to take them from.
Note that $K_1 \cap K_2$
For every $Y_1 \subset K_1$ and $Y_2 \subset K_2$ such that $K \ (Y_1 \cup Y_2) \subset \{\rho\}$ and $Y_1 \cap Y_2 = Y$, we find the solution such that $Y \cup Y_1$ is excluded in $T_1$ and $Y \cup Y_2$ is excluded in $T_2$.
...
The solution for that $Y$ is the merge of the best solutions for subsets $Y_1 \subset K_1$ and $Y_2 \subset K_2$ such that $K \ (Y_1 \cup Y_2) \subset \{\rho\}$ and $Y_1 \cup Y_2 = Y$.
So that could be a $2^{|K_1 \cap K_2|}$ operation.
We have to do this for every $(\rho, Y)$, meaning that the whole step will have complexity $K \times 2^{|K| + |K_1 \cap K_2|}$.


% A weaker but easier to state thing would be to have maps to subsets:
% Define the Constrained Maximum Agreement Taxon Subset (CMATS) problem for a given $(\chi, e, c)$ where $\chi$ is a leaf coloring and $e$ is an edge and $c \in \chit(e)$ be a taxon set $Y$ of maximal size such that $\chi$ restricted to $Y$ is convex and $\chi|_Y (e) = {c}$.
